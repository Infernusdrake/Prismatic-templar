...
func _build_environment() -> void:
	...
	env.background_color     = Color(0.05, 0.05, 0.10)
	env.ambient_light_source = Environment.AMBIENT_SOURCE_COLOR
	env.ambient_light_color  = Color(0.40, 0.40, 0.50)
...
func _connect_signals() -> void:
	...
	_enemy.execute_available.connect(_on_execute_available)
...
func _process(_delta: float) -> void:
	...
	# Execute input
	if Input.is_action_just_pressed("execute") and _enemy and not _enemy.is_dead:
		_enemy.attempt_execute(_player.global_position)
	...
	if _player:
		...
		if _enemy:
			_enemy.set_lock_ring(_player.locked_on)
...
func _on_enemy_died() -> void:
	_game_over = true
	_hud.show_message("ENEMY DEFEATED\n\nPress R to restart", 999.0)
